                        ...
                        <table class="table" style="color:#000;">
                            ...
                            <tbody>
                                ...
                                while($resultroom = mysqli_fetch_assoc($queryroom)){
                                    echo '<tr class="row" onclick="window.location=\'admin-update.php?memid=' . $resultroom['id'] . '\'">';
                                    echo '<td>' . $resultroom['fname'] . ' ' . $resultroom['lname'] . '</td>';
                                    echo '<td>' . $resultroom['status'] . '</td>';
                                    echo '<td>' . $resultroom['usertype'] . '</td>';
                    ...
                    <div class="container" style="background: linear-gradient(to left, rgb(247, 150, 150), #cebef8);width:1000px;">
                        <table class="table" style="color:#000;">
                            ...
                            <tbody>
                                ...
                                $queryroom = mysqli_query($conn, $sqlroom);
                                while($resultroom = mysqli_fetch_assoc($queryroom)){
                                    echo '<tr class="row" onclick="window.location=\'admin-update.php?eveid=' . $resultroom['id'] . '\'">';
...
<div class="content">
    <div class="container" style="background: linear-gradient(to left, rgb(247, 150, 150), #cebef8);margin-bottom: 400px;margin-left: 50px;">
        ...
        <form action="admin-update.php?eveid=<?php echo $eveid; ?>" method="post">
           ...
           <button style="display: flex;justify-content: center;align-items: center;margin:auto;border-radius:5px;background: linear-gradient(to left, rgb(247, 150, 150), #cebef8);color:#fff;margin-top:10px;padding:10px;" type="submit" name="update-event">Update Event</button>
        </div>
    </form>
        <?php
        if (isset($_POST['update-event'])){
            ...
            $eorg  = filter_input(INPUT_POST, "eorg", FILTER_SANITIZE_SPECIAL_CHARS);
                $sqlUpdate = "UPDATE event_tbl SET 
                                ename = '$ename', 
                                edesc = '$edesc', 
                                edate = '$edate', 
                                etime = '$etime', 
...
<div class="content" >
    ...
    <form action="admin-update.php?memid=<?php echo $memid; ?>" method="post">
           ...
           <button style="display: flex;justify-content: center;align-items: center;margin:auto;border-radius:5px;background: linear-gradient(to left, rgb(247, 150, 150), #cebef8);color:#fff;margin-top:10px;padding:10px;" type="submit" name="update-user">Update User</button>
        </div>
    </form>
        <?php
        if (isset($_POST['update-user'])){
            ...
            $section  = filter_input(INPUT_POST, "section", FILTER_SANITIZE_SPECIAL_CHARS);
                $sqlUpdate = "UPDATE memberstbl SET 
                                fname = '$fname', 
                                lname = '$lname', 
                                status = '$status', 
                                usertype = '$usertype', 
